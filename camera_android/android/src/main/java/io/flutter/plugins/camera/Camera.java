...
class Camera
  ...
  private boolean previewFrameSurfaceEnabled = false;
  ...
  /** For native board processing */
  private Map<String, Object> pendingBoardData;
  Messages.Result<Messages.PlatformCapturedImageData> flutterResultMemoryWithBoard;
  ...
  private void createCaptureSession(
    ...
    if (previewFrameSurfaceEnabled && imageStreamReader != null) {
  ...
  private void ensurePreviewFrameSurface() throws CameraAccessException, InterruptedException {
    if (previewFrameSurfaceEnabled) {
      return;
    }
    previewFrameSurfaceEnabled = true;
    startPreview(null);
  }

  /**
   * Convert PlatformBoardOverlayData to Map for processing
   * Native will calculate image coordinates after knowing actual size
   */
  private Map<String, Object> convertBoardDataToMap(Messages.PlatformBoardOverlayData data) {
    Map<String, Object> map = new HashMap<>();
    map.put("boardImageBytes", data.getBoardImageBytes());
    map.put("boardScreenX", data.getBoardScreenX());
    map.put("boardScreenY", data.getBoardScreenY());
    map.put("boardScreenWidth", data.getBoardScreenWidth());
    map.put("boardScreenHeight", data.getBoardScreenHeight());
    map.put("previewWidth", data.getPreviewWidth());
    map.put("previewHeight", data.getPreviewHeight());
  ...
  public void onImageAvailable(ImageReader reader) {
    ...
    if (flutterResultMemoryWithBoard != null && pendingBoardData != null) {
      // Capture to memory with board processing (native optimization)
      final Messages.Result<Messages.PlatformCapturedImageData> resultRef = flutterResultMemoryWithBoard;
      final Map<String, Object> boardDataRef = pendingBoardData;
      
      Log.i(TAG, "⚡ Starting native board processing");
      
      backgroundHandler.post(
          new BoardImageMemoryProcessor(
              image,
              boardDataRef,
              new BoardImageMemoryProcessor.Callback() {
                @Override
                public void onComplete(@NonNull byte[] bytes, int width, int height) {
                  ...
                  dartMessenger.finish(resultRef, result);
                  Log.i(TAG, "✅ Native board processing complete");
                }

                @Override
                public void onError(@NonNull String errorCode, @NonNull String errorMessage) {
                  Log.e(TAG, "❌ Native board processing failed: " + errorMessage);
                  dartMessenger.error(resultRef, errorCode, errorMessage, null);
                }
              }));
    ...
    } else if (flutterResultMemory != null) {
      // Capture to memory (standard)
      backgroundHandler.post(
  ...
  public void close() {
    ...
    previewFrameSurfaceEnabled = false;
    if (mediaRecorder != null) {
      mediaRecorder.reset();
      mediaRecorder.release();
      mediaRecorder = null;
